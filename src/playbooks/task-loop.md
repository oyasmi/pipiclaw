...
任务步骤默认静默；确有交付或需告知的变化才用 notify。若 Goal/DoD 要求“交付、告知、汇报、回复”结果，notify 必须包含要发给用户的实际内容；note 只是内部循环记录，不会发送给用户。note 只写真实做过什么、证据和下一步；不把每步过程追加到契约。Plan 可在 task_step_end 中一并更新。**每个任务步骤，包括 run/job 完成后的唤醒步骤，都必须以一次成功的 task_step_end 结束；读取到结果后直接输出普通文本会被丢弃，任务也不会结算。**
