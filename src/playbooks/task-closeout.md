...
FAIL 后用 progress 记录失败证据、status 回 in-progress，修复后重新 candidate。不要让 verifier 顺手修实现。

## 外部副作用

`sideEffects: external` 要求用户通过 `/tasks approve <id>` 授权；agent 不能自授予。授权绑定 task body hash，批准后不要再 progress 或编辑正文。

只做 external、无需 independent verification 时：准备动作 → progress 置 awaiting-user 并请求审批 → 用户 approve → 执行动作 → 直接 done。
...
3. 不改正文；用 `task_manage set status=awaiting-user wake=<合理时间>` 等待，并请求用户 `/tasks approve <id>`。这里不能 progress。
...
如果 wake 到期但用户尚未批准，只用 set 调整 awaiting-user/wake 并提醒，不写 progress note。
...
`task_manage done` 需要 summary 和可查证 evidence，可附 residualRisk。它检查 checkbox、依赖、child、外部授权、独立 PASS，再记录收尾、清理临时 task events；一次性任务归档，周期任务保留 `.schedule` 并原地睡眠。

放弃用 `cancel` + reason，不伪装 done。cancel 要求先处理 child，随后归档并删除包括 `.schedule` 在内的所有 task-owned events。
