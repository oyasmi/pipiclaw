...
`task_manage done` 需要 summary 和可查证 evidence，可附 residualRisk。它检查未勾选 checkbox、外部授权、独立 PASS、body/artifact hash，再记录收尾、清理临时 task events；一次性任务归档，有 `schedule` frontmatter 的周期任务算出下一次 `wake` 并原地睡眠。
...
放弃用 `cancel` + reason，不伪装 done。cancel 会归档任务并删除全部 task-owned events。
