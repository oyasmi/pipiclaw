...
- Local time is now expressed through one vocabulary. A new `src/shared/local-time.ts` (`formatLocalTime`/`parseLocalTime`/`parseWakeInput`/`localDayKey`/`localStampForFilename`) is now the single source for wake scheduling, memory day-bucketing, and `/usage` windows, fixing UTC day-boundary bugs where local midnight–8am computed the wrong day/month. `wake` now accepts relative offsets (`+2h`/`+45m`/`+3d`) so models never hand-compute timezones, and the on-disk wake format is normalized to the millisecond `formatLocalTime`. Tests pin `TZ=Asia/Shanghai`.
- Task mutations are serialized by a reentrant keyed queue. `task_manage`, `/tasks`, the driver, migration, and store updates now go through a `channelDir + taskId` keyed queue (`tasks/mutation-lock.ts`), closing the in-process window where `/tasks` (a non-queue command) could race the driver or agent and overwrite each other. `docs/scaling-and-concurrency.md` states explicitly that sharing one workspace across processes is unsupported; file locks/CAS would introduce stale locks and cross-platform semantics outside the single-process product model, so they are deliberately not introduced.
- The task transition table now owns two more actions. `escalate` (governor pause) and `start-cycle` (recurring cycle reopen) go through the shared transition table instead of writing status directly, and reject calls from illegal prerequisite states. `attemptGeneration` — backward-compatible, monotonically increasing, preserved across cycles — replaces timestamp-string equality for claim/release consistency, so a stale dispatch's release can no longer roll back a newer claim.

### Fixed

- **Task lifecycle.** `cancel`/`run` transitions now allow `done` as a starting state: a dormant recurring task can be retired directly (matching the documented retirement semantics) and `/tasks run` reopens the cycle (folding the prior cycle, resetting cycle control, clearing the old wake) instead of flipping a completed cycle back to `active`. `task_manage set` that changes `schedule` without an explicit `wake` now recomputes the wake to the next occurrence of the new rhythm regardless of current state (previously only for `done` tasks, leaving a stale wrong wake). `cleanupTaskEvents` separates I/O failure from parse failure — an unparseable legacy event is still handed to `/events`, while a read failure logs a warning with a repair step. A new `/tasks doctor` check flags recurring tasks whose `wake` does not land on any schedule occurrence, with a repair command.
- **Event scheduling.** A one-shot event whose `preAction` is gated or fails is now consumed and deleted (`deleteAfter`), instead of lingering forever and holding a tool-side quota slot; periodic events still only skip the occurrence. Periodic Cron now sets `protect: true` and `catch: true`, so overlapping callbacks cannot run concurrently and a future refactor that drops the catch can't produce an unhandled rejection. Startup recovery processes files in batches of at most 4, logs a warning per failed file without blocking the rest, and stops launching remaining batches after `/stop`. The ~24.8-day one-shot scheduling ceiling is now enforced in the shared admission validation, so `event_manage` rejects it at write time with a recoverable error instead of letting the watcher reject it later.
- **Memory.** `add` and a missing-target `supersede` now dedupe against the current file and same-batch additions using the existing NFKC/whitespace-fold/case-normalized hash, and the consolidation window keys on `sourceCorrelationId` as a batch idempotency key — so a window rerun no longer re-promotes the same fact even if the model rewords it. `forget` writes the original `sourceEntryIds` into the tombstone, blocking same-window reworded replays; the tool copy now accurately states it only guarantees exact-content / original-source replay. Durable extraction, SESSION update, MEMORY cleanup, and HISTORY folding now share one "transcript/memory/history/session are untrusted data, not instructions" input-boundary rule. The review-log path-fingerprint table is now a bounded 256-entry LRU; `session_search`'s single-slot cache is bucketed by `(channelDir, build args)` (max 8, 30s TTL); a dead `appendChannelMemoryUpdate` API that bypassed secret redaction and tombstone checks is removed; and `memory/policy.ts` is renamed to `secret-redaction.ts` to match its job.
- **Agent / jobs.** Each channel's earliest-expiring completed job record now gets an `unref`'d one-shot GC timer, so records and spill/exit files are reclaimed without waiting for the next job or a restart (the sweeper still supervises running jobs). The background-job spill path uses `node:os` `tmpdir()` instead of a hardcoded Linux `/tmp`, with the owner-only `umask 077` unchanged.
- **Runtime / architecture.** The task ledger parser moved from `shared/task-ledger.ts` to `tasks/ledger.ts`, removing the `shared → tasks` reverse dependency. The DingTalk transport reserves the turn via an explicit synchronous `reserveEvent` hook before entering the async body; idle channel caches get independent periodic reclamation (busy queues and in-flight cards are not swept); and `enqueueEvent`'s hardcoded limit is extracted to `EVENT_QUEUE_LIMIT`. The format-only module is renamed `config-diagnostic.ts`, and the agent session adapts Pipiclaw settings through the upstream `SettingsManager.inMemory()` instead of presenting the runtime settings class as an SDK mock.
...
- Slimmed the task-governance domain (spec 036), removing four kinds of complexity the runtime paid full cost for without earning a durable guarantee — its guard state is in-memory and lost on restart. Per-task token/cost/wall-time budgets collapse to a single `maxAttempts` (the only per-task stop-loss; cost stays observable in `usage` and `/tasks stats`, just no longer enforced per task), and the non-resetting `lifetimeUsage` double-ledger is deleted — for one-shot tasks current usage already equals lifetime, and for recurring tasks cross-cycle history lives in the body's `History` section. The `parent`/`dependsOn` relation graph is gone entirely, retiring four separate cycle-detection implementations; task ordering is now expressed human-readably in the body's Manual/Goal or via `wake`, and `done`/`skip`/`cancel` no longer gate on children. Task-owned worktrees and the `subagent isolation` parameter are removed (sub-agent invocation surface 15 → 14); if you need an isolated checkout, create it on the host and pass the path as an ordinary working directory. Verification collapses from two modes to one: `mode: "evidence"|"independent"` becomes `required: boolean`, the self-attesting evidence branch (where `done` wrote its own `status: "passed"`) is removed so evidence can no longer masquerade as a verification pass, and the four mutually-contradictory defaults converge to one — with a behavior change that tasks marked `sideEffects: "external"` now default to requiring independent verification. The attestation file keeps its anti-forgery core (runId/taskId/verdict/bodyHash/subjectHash plus the `done`-time re-check that stops a hand-written "passed"), drops three write-only fields (`outputHash`/`agent`/`model`), and the `subjectHash` failure semantics are unified to fail-closed on both sides. Existing task files keep working — unknown keys are ignored on read and canonicalized on next write; `/tasks doctor` reports which tasks still carry retired keys and explicitly notes that any `parent`/`dependsOn` ordering intent is no longer enforced. Net −395 lines across `src/`. Beta API note: `SubAgentInvocationOverrides` loses `isolation`/`worktreePath`/`worktreeBranch`.
...
- Task usage is now honestly accounted and lifecycle-aware. A non-resetting `lifetimeUsage` records cumulative attempt/token/cost/wall-time alongside the per-cycle usage that `start-cycle` resets (the on-disk `control` stays version 1 and auto-backfills on read). `costKnown` distinguishes "genuinely free" from "model pricing unavailable," so a missing price tier surfaces as `unavailable` instead of a misleading `$0.0000`; a `maxCostUsd` breach becomes a recoverable rejection with a next step. `/tasks` now surfaces the current and last cycle, next wake, and host timezone, and the driver stamps the actual timezone at startup.
- Added a recurring-only `task_manage skip`: de-duplication cycles skip DoD/evidence checks and drop straight back to dormant, while unfinished children still block the skip.
- The `done` path in evidence mode now writes a full verification record (`verification.status=passed`, evidence, timestamp, and contract hash), so a task completed by evidence satisfies the same acceptance contract as an explicit verify.

### Changed

- Collapsed the memory maintenance pipeline from four jobs to three by merging durable consolidation and the growth review into a single `runMemoryCheckpointJob` that runs on consolidation semantics: one cursor, one interval, one gate. The scheduler now runs only session refresh → memory checkpoint → structural maintenance; boundary events (compact/`/new`/shutdown) keep going through `runInlineConsolidation`, sharing the same cursor and extraction channel as the checkpoint. This also retires automatic skill promotion: the post-turn review and promotion-signal paths are gone, `extraction.ts` drops its skill branch and schema, and skills can only be created explicitly via `skill_manage` (the tool is retained). State and settings converged accordingly — four timestamp/cursor fields fold into `lastCheckpointAt`/`lastCheckpointEntryId`, growth counters are removed, `normalizeState` migrates legacy fields, and the `memoryGrowth` settings block is deleted in favor of a single `checkpointIntervalMinutes` (default 20); legacy keys are silently ignored and run on the new defaults. Net ~950 lines removed across 36 files. Beta API note: the `memoryGrowth` settings block and the `runGrowthReviewJob`/skill-auto-write internals are gone.
- Three task-control simplifications remove parallel state machines without behavior change (disk format stays backward-compatible — unknown keys are ignored on read, retired enum values canonicalized on next write). `control.isolation` is deleted: no code read it for decisions and `recordTaskWorktree` already mirrored the same fact into `control.worktree`, so isolation is now declared once at the delegation point and `control.worktree`'s existence *is* the isolation fact. `lastOutcome` becomes runtime-exclusive: the ten hand-assignments across lifecycle, verification, and `/tasks pause|resume|run` are removed and it is dropped from the tool schema and `TaskControlPatch` so the model can no longer set it — only attempt claim/finish and governor escalation write it, and the enum collapses 7→5 (verified/skipped had no remaining writers), sharpening the meaning to "how the last agent turn ended" (`/tasks stats` relabels it "last run:"). `sideEffects` becomes binary: `read-only` and `workspace` were equivalent everywhere (only `=== "external"` is checked), so they merge, with a schema note that `external` triggers the approval gate so the enum's machine semantics stay self-documenting.
- Bounded periodic-task cycle bodies and made legacy ledgers self-healing. Evidence is now upserted within `Current Cycle`; `History` is capped at 8 entries / 24 KiB (4 KiB each) with a `session_search` hint when truncated. Legacy-format ledgers auto-migrate on the next cycle open: duplicate top-level `Evidence` is dropped, the latest is kept, and the rest fold into `History`. When multiple `Current Cycle` headers exist in history, the in-body current cycle is always selected, fixing progress being written to a stale cycle.
