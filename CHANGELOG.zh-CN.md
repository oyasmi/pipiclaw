...
- 记忆写入的 review-log 条目现在记录真实的 durable/probationary 候选数（`durableCandidates`/`probationaryCandidates`），不再只记最终写入数——一次因去重而未实际写入的候选，之前会在 `memory-review.jsonl` 里留下一个空的 action 数组，现在可见。
- `MEMORY.md` 清理的收缩守卫现在也保护小文件（此前只在文件超过字符阈值后才检查条目数占比，导致"四个短 Update 块"——最常见的 cleanup 触发形态——完全不设防），并且会拒绝任何丢弃了用户明确要求保存的条目的清理输出。
- 记忆召回的 LLM 重排现在在选出空结果时保底返回本地排序第一名，而不是直接返回空——进入重排的候选本来就已经过了词法证据门槛，"重排选了空"更可能是过度谨慎而非真的无关。
- 记忆相关的 sidecar LLM 调用在超时/中止路径下现在也会记账用量，不再只在成功时记账——此前 `/usage` 的 sidecar 行会系统性漏记所有失败的召回重排/提炼调用产生的花费。
- `src/memory/review-log.ts` 源码中曾含一个裸 NUL 字节，导致 `git diff`、`rg` 等文本工具把该文件当作二进制文件处理，使其在不确定的时间段内实际上无法被 code review 覆盖。已替换为转义的 ` `（运行时字节完全一致）。

- `HISTORY.archive.md` 现在改成 append-only，超过 4MB 时轮转出一份 `.1`，不再每次历史折叠都全量读写一遍（此前是全生命周期 O(n²)，且没有大小上限）。
- 频道记忆备份（`.memory-backups/`）保留代数从 5 提到 10——按默认 6 小时一次的结构维护节奏，5 份只够覆盖约 30 小时。
