...
- gate `tools.jobs.enabled`（**默认开**；关掉即回到无后台作业的行为）：关闭时 bash 的 `async` 参数报错说明、`job` 工具不注册——完全回到未开启前的行为。
...
- gate `tools.bashInterceptor.enabled`（**默认开**；可在 `tools.json` 关掉）。规则当前硬编码为三条最明确的裸形态（可配规则表见 §实现说明，为后续细化）。
...
各 gate 的默认值：`grep`、`jobs`、`bashInterceptor` 均**默认开**（本次按用户决定统一开启；`jobs`/`bashInterceptor` 起初设计为灰度默认关，现已翻为默认开）。仍可在 `tools.json` 里各自关掉。
...
- 每个 wave 结束跑 `npm run check` 全量门 + 运营观察一段（channel telemetry / 维护日志），再进下一 wave。（`jobs`/`bashInterceptor` 起初计划灰度默认关；本次按用户决定统一翻为默认开，仍可按需在 `tools.json` 关闭。）
