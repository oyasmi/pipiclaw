...
具体场景：`weekly-report-reminder.json`配置了`action`字段，但Pipiclaw完全忽略该字段。每天16:00都触发LLM，LLM每天都回复"今天不是最后工作日"。

---

## 需求证据

- 用户已有完整的判断脚本`check-last-workday.js`（含中国法定节假日+调休逻辑），退出码0/1设计，但因为events系统不支持action字段而无法使用
...
- 向后完全兼容：没有`action`字段的事件行为不变
- 只支持bash command执行，不需要其他执行器
- 脚本执行必须有超时保护，不能阻塞事件调度
- `action.command`执行前需经过`guardCommand`安全检查，因为事件文件可由LLM通过文件工具动态创建，存在间接注入路径
- 需要覆盖测试
...
在三个独立interface（`ImmediateEvent`、`OneShotEvent`、`PeriodicEvent`）中各添加`action?: EventAction`字段。当前类型定义没有公共基类，需要在三处都加。

### 二、parseEvent扩展

提取通用的`parseAction(data)`辅助函数，在switch之前调用，返回`EventAction | undefined`。验证逻辑：如果`data.action`存在，检查`action.type === "bash"`且`action.command.trim().length > 0`（空字符串视为无效，抛出解析错误）。然后在三个case分支的返回对象中都附加`action`字段。
...
private async execute(filename, event, deleteAfter = true): Promise<void> {
  // Gate check: run action before enqueue
  if (event.action) {
    try {
      await this.runAction(event.action, filename);
    } catch {
      log.logInfo(`Action gate blocked event: ${filename}`);
      return;
    }
  }
...
private runAction(action, filename): Promise<void> {
  // Security gate
  const guardResult = guardCommand(action.command, this.securityConfig.commandGuard);
  if (!guardResult.allowed) {
    log.logWarning(`Action command blocked by guard: ${guardResult.reason}`);
...
- action退出码1 → 事件被跳过
- action超时 → 事件被跳过
- action命令不存在/路径错误 → 事件被跳过
- `action.command`为空字符串 → parseEvent抛出解析错误
- `action.command`被guardCommand拦截 → 事件被跳过，记录warning日志
- 无action字段 → 行为不变（回归测试）

### 五、文档更新

更新`docs/events-and-sub-agents.md`中的事件配置说明，增加action字段文档。

---

## 待决事项

1. action执行的默认超时设为10秒（gate检查应该快速完成），可通过timeout字段覆盖
2. action执行失败（非0退出码）不写error marker——这是正常的"gate closed"行为

---

## 成功标准

- `weekly-report-reminder.json`配置action后，非最后工作日不再触发LLM会话
- 所有现有无action字段的事件行为不变
- 测试覆盖gate通过和被阻断两条路径
