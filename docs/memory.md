...
**改主意时可以直接说新的一句**，不必自己先说"忘掉旧的"：`memory_manage save` 在写入前会先按关键词查一遍频道已有记忆，如果发现高度相似的已有条目，会先停下来把冲突条目列出来，再决定是替换（supersede）还是两条都保留（两件事同时成立时）。仍然可以主动说"忘掉之前说的 XX"来跳过这一步直接删除。
...
**"它记得，但要我把关键词说得刚刚好才想得起来。"** 这曾经是真的：早期的召回按"命中词数 ÷ 提问词数"打分，你说得越详细分母越大，反而越难命中。现在改成按命中词的**特异性**累加算证据，与提问长度无关——把背景讲清楚不再有惩罚。仍然可能漏的是**完全换一套说法**的情况（例如记忆里写的是"用 pnpm"，你问"依赖装一下"，两句话没有任何共同词）；这种时候直接点名主题，或让 agent 用 `memory_manage search` 去找。**追问式的指代**（"上次说的那个安排，代号是什么来着？"）现在也能命中——这句话本身全是代词和虚词，召回会借用上一轮你说的话来判断该找什么，只在追问自己不可能命中时才会这样做，正常提问不受影响。
