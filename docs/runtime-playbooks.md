...
| `task-recurring.md` | 单文件 + `schedule` frontmatter、runtime 自动开新周期、调节奏、退役 |
...
| `task-repair.md` | 治理器暂停（paused/governor）、停滞、坏 frontmatter、stale gate、孤儿事件、重启恢复 |
...
6. **与代码共同验证**：metadata/catalog、prompt 不加载正文、path guard 和关键组合状态机都有测试；构建产物（`dist/playbooks/` 只含 `.md`）按 build 脚本保证，发版前人工核对。
