...
- **瘦身与内容责任（spec 026）**：Pipiclaw 只为自己写下的固定段负责，且严格克制——不再重复 tool 目录（tool schema 已是权威），identity/contract/boundary 文案压缩，playbook 目录只保留 filename + 一句 trigger 并在头部打印一次绝对 `PLAYBOOKS_DIR`，无预定义 sub-agent 时整段消失。当前 full-tools 下 runtime-authored ≈ 390 prompt units（旧基线 ~1047）。
