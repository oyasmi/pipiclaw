...
spec 051 把任务拆成三样东西：**契约**（`tasks/<id>.md`，硬上限 4 KB，每一步完整注入）、**循环日志**（`tasks/<id>.jsonl`，append-only）和**等待票**（frontmatter 的 `ticket`）。一次停泊必须说清楚什么会叫醒它，运行时在写入时校验（run/job 存在且未结算且归属本任务），并确定性地补上 `by` 兜底时限；到点未兑现就重开任务，同周期第二次就停下并通知用户。每个 cycle 在自己的会话（`tasks/.sessions/`）里跑，四维预算（步数/墙钟/成本/返工轮次）取代了原来的指纹 + effect 账本治理器。
