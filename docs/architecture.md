...
| `src/agent/` | 单频道 agent 编排：组装 SDK 会话、跑一轮、流式回传 | `channel-runner.ts`（核心编排器）、`session-events.ts`、`prompt-builder.ts`、`model-fallback.ts` |
...
其它硬化：六个可能含密钥的配置文件（`channel/auth/models/settings/tools/security.json`）创建即 0600，启动时对已存在的宽权限文件收紧；系统提示词层面还有"外部副作用需显式授权"等常驻不变量（`prompt-builder.ts`）。
...
系统提示词的组装顺序（`channel-runner.ts` 的 `appendSystemPromptOverride` + `prompt-builder.ts`）：`SOUL.md` → SDK 基础段 → 运行时段（Runtime 环境、知识分层边界、资源地图、常驻不变量、工具索引、任务核心规则、playbook 目录、子代理列表）。playbook 正文不进提示词，agent 触发时用 `read` 按需加载。
