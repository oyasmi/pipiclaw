...
| 当前 channel 目录 | 允许 | 仅 `tasks/` | 频道的记忆与任务台账不随项目切换失联；`memory/*.md`（生成的 `MEMORY.md` 索引）由后台维护队列持有，写入走 `memory_save` / `memory_forget`；`journal/` 只由后台反思写，不接受任何工具写入；任务正文是 agent 自己写的契约，需要 `edit` 修订 |
